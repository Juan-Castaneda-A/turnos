...
let navLinks, contentSections, turnsWaitingElement, turnsAttendedTodayElement, activeModulesElement,
    ...
    modalMessage, modalConfirmBtn, modalCancelBtn, moduleFilter, chatModal, openChatBtn, closeChatBtn, chatRoomList, chatMessageHeader,
    chatMessageArea, chatMessageForm, chatMessageInput, chatSendBtn,
    notificationList, kpiTurnosAtendidos, kpiTiempoPromedio;
...
async function loadUserCache() {
    try {
        ...
        console.log("Caché de usuarios (Admin) cargado.");
    } catch (error) {
        console.error("Error al cargar caché de usuarios (Admin):", error.message);
    }
}

async function openChat() {
    console.log("Abriendo chat (Admin)...");
...
async function loadChatRooms() {
    ...
    try {
        ...
        result.rooms.forEach(room => {
            ...
            roomElement.onclick = () => { selectChatRoom(room.id, room.nombre); };
            chatRoomList.appendChild(roomElement);
        });
        const divider = document.createElement('hr');
        divider.className = 'border-gray-700 my-2';
        chatRoomList.appendChild(divider);
        if (userCacheMap.size === 0) { await loadUserCache(); }
        userCacheMap.forEach((userName, userId) => {
            ...
            userElement.onclick = () => { openDirectMessage(userId, userName); };
            chatRoomList.appendChild(userElement);
        });
    } catch (error) {
        console.error("Error al cargar salas de chat (Admin):", error.message);
...
async function loadChatMessages(roomId) {
    try {
        ...
        if (result.messages.length === 0) {
            chatMessageArea.innerHTML = '<div class="flex justify-center items-center h-full"><p class="text-gray-500">No hay mensajes.</p></div>';
            return;
        }
...
function renderMessage(message, isMe) {
    ...
    chatMessageArea.style.gap = '0.75rem';
...
async function handleSendMessage(event) {
    ...
    const messagePayload = { type: 'text', content: content };
    sendStructuredMessage(messagePayload); // Usamos la función de stickers
    chatMessageInput.value = '';
}

async function sendStructuredMessage(payload) {
    if (!currentChatRoomId || !payload) return;
    chatMessageInput.disabled = true;
    chatSendBtn.disabled = true;
    try {
        ...
        console.error("Error al enviar mensaje estructurado:", error.message);
    } finally {
        chatMessageInput.disabled = false;
        chatSendBtn.disabled = false;
        chatMessageInput.focus();
...
function setupRealtimeSubscriptions() {
    ...
    const chatMessagesChannel = supabase.channel('admin_chat_messages_channel');
    chatMessagesChannel.on(
        ...
        (payload) => {
            console.log("Nuevo mensaje de chat recibido (Admin):", payload.new);
            const msg = payload.new;
            const isMe = msg.sender_id === window.USER_ID;
            if (isMe) return; // No notifico mis propios mensajes
    ...
    const chatParticipantsChannel = supabase.channel('admin_chat_participants_channel');
    chatParticipantsChannel.on(
        ...
        (payload) => {
            console.log("¡Me han añadido a una nueva sala de chat! (Admin)", payload.new);
